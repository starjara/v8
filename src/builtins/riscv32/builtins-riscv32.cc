...
static void GenerateTailCallToReturnedCode(MacroAssembler* masm,
  ...
  // -----------------------------------
  {
    ...
    __ Add(a2, a0, Operand(Code::kHeaderSize - kHeapObjectTag));
...
void Generate_PushArguments(MacroAssembler* masm, Register array, Register argc,
  ...
  Label loop, entry;
  __ Sub(scratch, argc, Operand(kJSArgcReceiverSlots));
  __ Branch(&entry);
  __ bind(&loop);
...
void Builtins::Generate_ResumeGeneratorTrampoline(MacroAssembler* masm) {
  ...
  __ Sw(a0, FieldMemOperand(a1, JSGeneratorObject::kInputOrDebugPosOffset));
  ...
  __ Lw(a4, FieldMemOperand(a1, JSGeneratorObject::kFunctionOffset));
  __ Lw(cp, FieldMemOperand(a4, JSFunction::kContextOffset));
  ...
  // will never be used.
  __ Lw(a3, FieldMemOperand(a4, JSFunction::kSharedFunctionInfoOffset));
  __ Lhu(a3,
         FieldMemOperand(a3, SharedFunctionInfo::kFormalParameterCountOffset));
  __ Sub(a3, a3, Operand(kJSArgcReceiverSlots));
  __ Lw(t1,
        FieldMemOperand(a1, JSGeneratorObject::kParametersAndRegistersOffset));
  {
    Label done_loop, loop;
    __ bind(&loop);
    __ Sub(a3, a3, Operand(1));
  ...
  {
    __ Lw(a0, FieldMemOperand(a4, JSFunction::kSharedFunctionInfoOffset));
...
#undef JUMP_IF_EQUAL
  ...
  __ Add(scratch2, bytecode_size_table, bytecode);
...
void Builtins::Generate_InterpreterEntryTrampoline(MacroAssembler* masm) {
  ...
  __ Lw(a4, FieldMemOperand(feedback_vector, HeapObject::kMapOffset));
  ...
  Label stack_overflow;
  {
    ...
    __ bind(&loop_check);
    __ Sub(a4, a4, Operand(kSystemPointerSize));
  ...
  __ bind(&is_baseline);
  {
    ...
    // allocate it.
    __ Lw(scratch, FieldMemOperand(feedback_vector, HeapObject::kMapOffset));
...
void Builtins::Generate_InterpreterPushArgsThenCallImpl(
  ...
  if (mode == InterpreterPushArgsMode::kWithFinalSpread) {
    ...
    __ Sub(a0, a0, Operand(1));
  }

  if (receiver_mode == ConvertReceiverMode::kNullOrUndefined) {
    __ Sub(a3, a0, Operand(kJSArgcReceiverSlots));
  } else {
    __ Move(a3, a0);
  }
...
void Builtins::Generate_InterpreterPushArgsThenConstructImpl(
  ...
  __ Sub(argc_without_receiver, a0, Operand(kJSArgcReceiverSlots));
...
static void Generate_InterpreterEnterBytecode(MacroAssembler* masm) {
  ...
  __ Add(t0, t0, Operand(Code::kHeaderSize - kHeapObjectTag));
...
void OnStackReplacement(MacroAssembler* masm, bool is_interpreter) {
  ...
  __ Lw(a1, MemOperand(a0, Code::kDeoptimizationDataOrInterpreterDataOffset -
                               kHeapObjectTag));
...
void Builtins::Generate_FunctionPrototypeApply(MacroAssembler* masm) {
  ...
  // present) instead.
  {
    ...
    UseScratchRegisterScope temps(masm);
    Register scratch = temps.Acquire();
    __ Sub(scratch, argc, JSParameterCount(0));
...
void Builtins::Generate_ReflectApply(MacroAssembler* masm) {
  ...
  // thisArgument (if present) instead.
  {
    ...
    UseScratchRegisterScope temps(masm);
    Register scratch = temps.Acquire();
    __ Sub(scratch, argc, Operand(JSParameterCount(0)));
    __ Branch(&done0, ne, scratch, Operand(zero_reg), Label::Distance::kNear);
...
void Generate_AllocateSpaceAndShiftExistingArguments(
  ...
  __ Sub(sp, sp, Operand(new_space));
...
void Builtins::Generate_CallOrConstructVarargs(MacroAssembler* masm,
  ...
  // Push arguments onto the stack (thisArgument is already on the stack).
  {
    ...
    __ bind(&loop);
    __ Lw(a5, MemOperand(src));
    __ Add(src, src, kTaggedSize);
...
void Builtins::Generate_CallOrConstructForwardVarargs(MacroAssembler* masm,
  ...
  if (mode == CallOrConstructMode::kConstruct) {
    ...
    __ Lw(scratch, FieldMemOperand(a3, HeapObject::kMapOffset));
...
void Builtins::Generate_CallBoundFunctionImpl(MacroAssembler* masm) {
  ...
  // Patch the receiver to [[BoundThis]].
  {
    UseScratchRegisterScope temps(masm);
    Register scratch = temps.Acquire();
    __ Lw(scratch, FieldMemOperand(a1, JSBoundFunction::kBoundThisOffset));
...
void Builtins::Generate_ConstructFunction(MacroAssembler* masm) {
  ...
  __ Lw(a4, FieldMemOperand(a1, JSFunction::kSharedFunctionInfoOffset));
...
void Builtins::Generate_Construct(MacroAssembler* masm) {
  ...
  Register scratch = temps.Acquire();
  __ Lw(map, FieldMemOperand(a1, HeapObject::kMapOffset));
  __ Lbu(scratch, FieldMemOperand(map, Map::kBitFieldOffset));
  __ And(scratch, scratch, Operand(Map::Bits1::IsConstructorBit::kMask));
...
void Builtins::Generate_CEntry(MacroAssembler* masm, int result_size,
  ...
  if (argv_mode == ArgvMode::kRegister) {
    ...
    __ Sub(s1, s1, kSystemPointerSize);
  }
...
void Builtins::Generate_DoubleToI(MacroAssembler* masm) {
  ...
  __ Sub(scratch, result_reg, HeapNumber::kExponentMask);
  ...
  result_reg = sign;
  sign = no_reg;
  __ Sub(result_reg, zero_reg, input_high);
...
void Builtins::Generate_CallApiCallback(MacroAssembler* masm) {
  ...
  __ Add(a0, sp, Operand(1 * kSystemPointerSize));
...
void Generate_DeoptimizationEntry(MacroAssembler* masm,
  ...
  // frame description.
  __ Add(a3, a1, Operand(FrameDescription::frame_content_offset()));
  Label pop_loop;
